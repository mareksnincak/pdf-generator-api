...
GitHub Actions runs lint, unit tests, and integration tests on every pull request. On merge to `main`, the pipeline deploys via CDK and then runs E2E tests against the live (dev) environment - E2E tests run directly after deploy and use variables generated by deploy. See [`.github/workflows/ci-cd.workflow.yml`](.github/workflows/ci-cd.workflow.yml).
